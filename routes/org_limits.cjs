...
try {
  ({ pool } = require('../lib/db'));
  ({ requireAuth } = require('../lib/auth'));   // keep your existing auth middleware
} catch (e) {
  console.error('Import paths need adjusting?', e.message);
  // fallback for smoke tests only; comment/remove in prod:
  requireAuth = (req, _res, next) => next();
}

/**
 * GET /organizations/:id/limits
 * Returns saved plan & caps for the org.
 */
