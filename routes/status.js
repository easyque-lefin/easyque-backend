 ...
 * Response:
 *  {
 *    org: {..., org_banner_url, google_review_url},
 *    booking: {...},       // may be null
 *    metrics: { now_serving_token, avg_service_seconds, break_until, state, ... }
 *  }
 */
router.get('/view', async (req, res, next) => {
  try {
    ...
    const [orgRows] = await db.query(
      `SELECT id, name, address, map_url, org_banner_url, banner_url, google_review_url
    ...
    if (booking_id) {
      const [b] = await db.query(
        `SELECT id, org_id, assigned_user_id, token_number, status, customer_name, customer_phone, created_at
         FROM bookings WHERE id = ? AND org_id = ? LIMIT 1`, [booking_id, org_id]);
      booking = b[0] || null;
    } else if (token) {
      if (phone) {
        const [b] = await db.query(
          `SELECT id, org_id, assigned_user_id, token_number, status, customer_name, customer_phone, created_at
           FROM bookings WHERE org_id = ? AND token_number = ? AND customer_phone = ? 
           ORDER BY id DESC LIMIT 1`, [org_id, token, phone]);
        booking = b[0] || null;
      } else {
        const [b] = await db.query(
          `SELECT id, org_id, assigned_user_id, token_number, status, customer_name, customer_phone, created_at
           FROM bookings WHERE org_id = ? AND token_number = ? 
           ORDER BY id DESC LIMIT 1`, [org_id, token]);
        booking = b[0] || null;
      }
    }

    // METRICS — from assigned_live_metrics; prefer the assignee of the booking, else org rollup
    let metrics = null;
    if (booking?.assigned_user_id) {
      const [m] = await db.query(
        `SELECT org_id, assigned_user_id, now_serving_token, avg_service_seconds, 
         ...
         ORDER BY updated_at DESC LIMIT 1`, [org_id, booking.assigned_user_id]);
      metrics = m[0] || null;
    }
    if (!metrics) {
      const [m] = await db.query(
    ...
    // Compute state + ETA (best-effort; keep simple)
    if (metrics) {
      const now = Date.now();
      ...
      if (booking?.token_number && metrics.now_serving_token != null) {
