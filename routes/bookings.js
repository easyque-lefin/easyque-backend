...
router.get('/', async (req, res) => {
  try {
    ...
    // Return DB error text in dev to help debugging (remove in production)
...
router.post('/', async (req, res) => {
  try {
    ...
    const insertId = r && (r.insertId || (Array.isArray(r) && r[0] && r[0].insertId)) ? (r.insertId || r[0].insertId) : null;
